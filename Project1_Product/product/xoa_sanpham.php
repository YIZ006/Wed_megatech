...
if ($_SERVER['REQUEST_METHOD'] !== 'GET') {
    header("Location: /Project1_Product/product/quanly_sanpham.php?error=invalid_method");
    exit;
}

if (!isset($_GET['masp'])) {
    header("Location: /Project1_Product/product/quanly_sanpham.php?error=missing_id");
    exit;
}
...
if (mysqli_num_rows($result_exist) === 0) {
    header("Location: /Project1_Product/product/quanly_sanpham.php?error=not_found");
    exit;
}

// Kiểm tra ràng buộc: có sản phẩm liên quan không
$check_sql = "SELECT COUNT(*) AS total FROM sanpham WHERE masp = ?";
...
if ($row['total'] > 0) {
    header("Location: /Project1_Product/product/quanly_sanpham.php?error=linked");
    exit;
}
...
if (mysqli_stmt_execute($stmt)) {
    header("Location: /Project1_Product/product/quanly_sanpham.php?success=deleted");
} else {
    header("Location: /Project1_Product/product/quanly_sanpham.php?error=delete_failed");
}
exit;
?>
